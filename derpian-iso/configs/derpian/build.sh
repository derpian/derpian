...
make_basefs() {
	echo mkarchiso ${verbose} -w "${work_dir}" -C "${pacman_conf}" -D "${install_dir}" init
    mkarchiso ${verbose} -w "${work_dir}" -C "${pacman_conf}" -D "${install_dir}" init
    echo mkarchiso ${verbose} -w "${work_dir}" -C "${pacman_conf}" -D "${install_dir}" -p "haveged intel-ucode memtest86+ nbd" install
    mkarchiso ${verbose} -w "${work_dir}" -C "${pacman_conf}" -D "${install_dir}" -p "haveged intel-ucode memtest86+ nbd" install
}

# Additional packages (root-image)
make_packages() {
	echo mkarchiso ${verbose} -w "${work_dir}" -C "${pacman_conf}" -D "${install_dir}" -p "`echo $(grep -h -v ^# ${script_path}/packages.both)`" install
...
echo "ALL BAD((("
sed -i "s/ pcmcia / /g" "${work_dir}/root-image/etc/mkinitcpio-archiso.conf"
...
make_syslinux() {
    ...
    #gzip -c -9 ${work_dir}/root-image/usr/lib/modules/*-ARCH/modules.alias > ${work_dir}/iso/${install_dir}/boot/syslinux/hdt/modalias.gz && echo ok || echo bad
}
...
#        	sed -i 's|^Exec=|Exec=sudo -E |g' ${work_dir}/root-image/usr/share/applications/gparted.desktop
        ...
        part_five() {
            ...
            # FASTER
        #	mkarchiso ${verbose} -w "${work_dir}" -C "${pacman_conf}" -D "${install_dir}" \
        #    	-r '/usr/bin/sh /etc/firstrun.sh' \
        #    	run

        	mkarchiso ${verbose} -w "${work_dir}" -C "${pacman_conf}" -D "${install_dir}" \
            	-r '/usr/bin/touch /etc/firstrun1' \
            	run
        
        	mkarchiso ${verbose} -w "${work_dir}" -C "${pacman_conf}" -D "${install_dir}" \
            	-r 'systemctl -fq enable slim' \
            	run
...
make_common_single() {
echo "make_common_single!"
echo "make_basefs123!"
    run_once make_basefs
	echo mkarchiso ${verbose} -w "${work_dir}" -C "${pacman_conf}" -D "${install_dir}" init
    echo mkarchiso ${verbose} -w "${work_dir}" -C "${pacman_conf}" -D "${install_dir}" -p "haveged intel-ucode memtest86+ nbd" install
#ln -s "${work_dir}/airootfs" "${work_dir}/root-image"
cd work
ln -s airootfs root-image
cd ..

echo "make_packages!"
    run_once make_packages
echo "make_setup_mkinitcpio!"

    run_once make_setup_mkinitcpio
echo "make_customize_root_image!"
    run_once make_customize_root_image
echo "make_iso_version_files!"
    run_once make_iso_version_files
echo "build_kernel_modules_with_dkms!"
    run_once build_kernel_modules_with_dkms
echo "make_boot!"
    run_once make_boot
echo "make_boot_extra!"
